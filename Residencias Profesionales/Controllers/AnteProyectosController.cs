...
namespace Residencias.Controllers {
    ...
    [Authorize(Roles = "administrador,estudiante,docente,vinculacion")]
    public class AnteProyectosController : Controller {
        private ApplicationDbContext db = new ApplicationDbContext();
        private List<SelectListItem> opciones = new List<SelectListItem>();
        private UserManager<ApplicationUser> userManager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(new ApplicationDbContext()));

        protected override void Initialize(RequestContext requestContext) {
            base.Initialize(requestContext);
            string userid = requestContext.HttpContext.User.Identity.GetUserId();
            if (userManager.IsInRole(userid, "administrador")) {
                opciones.Add(new SelectListItem { Value = "0", Text = "Propuesta Propia" });
                opciones.Add(new SelectListItem { Value = "1", Text = "Situación como Trabajador/a" });
                opciones.Add(new SelectListItem { Value = "2", Text = "Banco de Proyectos" });
            } else if (userManager.IsInRole(userid, "estudiante")) {
                opciones.Add(new SelectListItem { Value = "0", Text = "Propuesta Propia" });
                opciones.Add(new SelectListItem { Value = "1", Text = "Situación como Trabajador/a" });
            } else if (userManager.IsInRole(userid, "docente") || userManager.IsInRole(userid, "vinculacion")) {
                opciones.Add(new SelectListItem { Value = "2", Text = "Banco de Proyectos" });
            }
        }
